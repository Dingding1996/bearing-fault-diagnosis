...
|---|---|---|---|---|
| 轴承 | 电机/传动系统 | 表面剥落/裂纹 | 轴承特征频率（BPFO/BPFI），包络分析最有效 | 微弱边带，不明显 |
| 转子 | 电机 | 不平衡 | 1倍转频能量突出 | 不明显 |
| 转子 | 电机 | 断条 | 转频谐波增大 | 工频两侧出现 ±2sf 边带，非常明显 |
| 定子 | 电机 | 匝间短路 | 不对称磁拉力引起轻微振动 | 三相电流不对称，特定次数谐波 |
| 气隙 | 电机 | 静态偏心 | 转频相关谐波 | 特定次数谐波 |
| 气隙 | 电机 | 动态偏心 | 转频相关谐波 | 旋转边带 |
| 联轴器 | 传动系统 | 不对中 | 2倍转频突出，轴向振动大 | 不明显 |
| 齿轮 | 传动系统 | 磨损/断齿 | 啮合频率（转速×齿数）及谐波 | 几乎看不到 |
| 轴 | 传动系统 | 弯曲/裂纹 | 1倍和2倍转频异常 | 不明显 |

> 本项目聚焦于**轴承故障**——振动信号的包络分析是最有效的检测手段；电流信号对轴承故障不敏感（边带极弱），但对转子断条等电机内部故障非常有效。

**检测什么？**
轴承故障会在振动和电流信号中产生冲击性脉冲——峰度升高、波峰因数增大，以及在特征缺陷频率（BPFO、BPFI）处出现能量峰值。
...
二分类任务（第 6 节）直接对应部署问题：*"这个轴承需要检修吗？"*
...
BPFO/BPFI/BSF/FTF 包络幅值也在每个文件对应的速度校正特征频率处单独计算。
...
| 域 | 特征 | 测量内容 |
|---|---|---|
| 时域 | RMS | 总体信号能量 — 轴承退化时第一个上升的指标 |
| 时域 | Peak | 最大绝对幅值 — 对突发大瞬变敏感 |
...
| 频域 | PSD 频带能量 | 特定频段的信号功率 — 表示为比值时与负载无关 |
| 频域 | 主频（Dominant frequency） | 最大能量处的频率 — 检测故障引入的新周期分量 |
| 频域 | WPD 子带能量 | 8 个等宽频率子带（第 3 级分解）中的信号能量 — 无需精确缺陷频率即可捕获故障能量在频谱上的分布方式 |
| 包络（仅振动） | BPFO/BPFI/BSF/FTF 幅值（1×–3× 谐波） | 解调包络中各缺陷频率的强度 — 直接量化故障冲击重复率 |
...
**包络频带选择（`ENVELOPE_BAND` 常量）：**
振动包络分析的带通滤波范围由笔记本第 0 节的 `ENVELOPE_BAND` 常量控制。
正确的频带必须以试验台的结构共振为中心——故障冲击激励此共振，在该频率处提升能量，从而实现清晰的解调。
第 3g-ii.5 节提供了全频带 PSD 检查，用于直观识别共振区域。更改 `ENVELOPE_BAND` 会自动使特征缓存失效并触发重新计算。
...
| BSF | 球自旋频率 | 31.9 Hz | 单个滚动体（球）的自转频率 |
| FTF | 保持架基频 | 9.5 Hz | 滚珠保持架的旋转频率 |
...
**主要指标**：
统一使用 **F1**（二分类）和 **F1-macro**（多分类，各类等权重）作为核心指标。
...
无监督模型仅在健康样本上训练，无需任何故障标签，两者 F1 均超过 0.80 的基线目标。
AUC-PR（IF = 0.918，OC-SVM = 0.920）均显著高于随机基线（0.714），验证异常分数确实学到了有效信号。
...
**FastAPI + Docker** — sklearn Pipeline 作为 REST API 提供服务。新的 `.mat` 文件直接传入；流水线在内部处理信号标准化、DSP 特征提取和预测。

**CI/CD** — GitHub Actions：每次推送时运行单元测试，合并到 `main` 时构建 Docker 镜像 → 推送到 ECR → 部署到 Elastic Beanstalk。

**AWS 基础设施：**

| 服务 | 角色 |
|---|---|
| S3 | 存储 MLflow 模型制品（`mlruns/`）和 EB 部署包 |
| ECR（弹性容器注册中心） | 私有 Docker 注册表 — 每次推送到 `main` 时构建以提交 SHA 标记的新镜像 |
| Elastic Beanstalk（eu-west-1） | 在 Nginx 反向代理后运行 FastAPI 容器；将端口 80 路由到容器端口 8000 |

**合并到 `main` 时的部署流程：**
1. 单元测试通过（pytest）
2. `mlruns/` 从 S3 同步 → 在构建时烘焙到 Docker 镜像中
3. 镜像以 `<commit-sha>` 和 `latest` 标签推送到 ECR
4. `Dockerrun.aws.json` 更新为新镜像 URI → 与 `.platform/` Nginx 配置打包 → 上传到 S3
5. EB 从 S3 包创建新应用版本并滚动部署到环境
...
└── mlruns/                       # MLflow 追踪 + 模型注册（已 gitignore）
```
